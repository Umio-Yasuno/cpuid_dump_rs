...
pub fn l1_amd_80_05h(tmp: CpuidResult) {
    print!(" [L1D {}K/L1I {}K]", tmp.ecx >> 24, (tmp.edx >> 24) & 0xFF);
    print!(
        "{} [L1dTLB: 4K {:>3}, 2M/4M {:>3}]",
        padln!(),
        (tmp.ebx >> 16) & 0xFF,
...
pub fn l2_amd_80_06h(tmp: CpuidResult) {
    print!(" [L2 {}K/L3 {}M]", (tmp.ecx >> 16), (tmp.edx >> 18) / 2);
    ...
    print!("{}{:9} 4M {:4}]", padln!(), "", (tmp.eax & 0xFFF) / 2);
}

pub fn l1l2tlb_1g_amd_80_19h(eax: u32, ebx: u32) {
    print!(
        " [L1TLB 1G: Data {:>3}, Inst {:>3}]",
        (eax >> 16) & 0xFFF,
        eax & 0xFFF
    );
    print!(
...
pub fn apmi_amd_80_07h(edx: u32) {
    ...
    let buff = format!("{0}{1}", has_ftr!(cpb, "CPB "), has_ftr!(rapl, "RAPL "),);
